        ...
        <v-card>
            ...
            <v-card-text>
                ...
                <v-container row wrap v-if="selectedSchema.name">
                    ...
                    <text-with-label label="Schema ID" :value="selectedSchema.schemaId"></text-with-label>
                    <text-with-label label="Credential Definition ID"
                                     :value="selectedSchema.credDefId"></text-with-label>
                    <text-with-label label="Revocation" :value="selectedSchema.revocable"></text-with-label>
                    <p class="title mb-0 ">Credential Fields</p>
                    <v-divider class="mb-3"/>
                    <text-with-label v-for="field in selectedSchema.attributes" :label="field.split('@')[0]"
                                     :value="(field.split('@')[1]).toLowerCase()"></text-with-label>
                </v-container>
            </v-card-text>
            <v-card-actions>
                <v-spacer/>
